...
def get_data_for_month(year, month):
    ...
    for row in rows:
        ...
        insider_data = {key: cols[index].find('a').text.strip() if cols[index].find('a') else cols[index].text.strip() 
                        for index, key in enumerate(['Symbol', 'Filing Date', 'Trade Date', 'Ticker', 'Company Name',
...
def get_openinsider_data():
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for year in range(2013, current_year + 1):
            start_month = 1 if year != 2024 else 3
        ...
        field_names = ['Symbol', 'Filing Date', 'Trade Date', 'Ticker', 'Company Name', 'Insider Name', 'Title',
